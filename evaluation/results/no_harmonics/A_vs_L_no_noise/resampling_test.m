...
[x, AErrSigFFTWin] = qwtbvar('plot2D', jobfn, 'L.v', 'AErrSigFFTWin.v');
% signal amplitude using sine fitting:
[x, AErrSigFit] = qwtbvar('plot2D', jobfn, 'L.v', 'AErrSigFit.v');
% signal amplitude using algorithm for signal frequency estimate:
[x, AErrSigEst] = qwtbvar('plot2D', jobfn, 'L.v', 'AErrSigEst.v');
% signal amplitude using fft of resampled signal:
[x, AErrResSigFFT] = qwtbvar('plot2D', jobfn, 'L.v', 'AErrResSigFFT.v');
...
hold off
% xlabel('Signal f (Hz)')
xlabel('Record length L (samples)')
