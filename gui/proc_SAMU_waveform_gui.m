function proc_SAMU_waveform_gui() %<<<1
...
% successful installation is shown.
    %% define globals %<<<3
    global GUIname; % defined in proc_SAMU_waveform_gui()
    global udata;   % structure with user input data
...
% open web page, hardcoded
    web('https://github.com/KaeroDot/EPMDigitalITsw/blob/main/gui/proc_SAMU_waveform_gui_help.md', '-browser')
...
% loads data from a datafile. several types supported.
    ...
    try
        ...
        errmsg = sprintf('Error: Trying to load data file `%s` resulted in following error:\n%s', filename, ERR.message);
        errordlg(errmsg, GUIname)
        error(errmsg);
    end % try_catch
    ...
    % check loaded data:
    if isempty(y)
        errmsg = sprintf('Error: No data in data file `%s`.', udata.datafile);
        errordlg(errmsg, GUIname)
        error(errmsg);
    end
    if not(isnumeric(y))
        errmsg = sprintf('Error: data in data file `%s` are not numeric.', udata.datafile);
        errordlg(errmsg, GUIname)
        error(errmsg);
    end
...
% loads csv file generated by wireshark exporting pcap file to csv
    ...
    % read 8 values separated by comma, all enclosed by ""
    data = textscan(fid, '%q %q %q %q %q %q %q %q', 'Delimiter', ',');
    fclose(fid);
    % must be at least 8 collumns
    if numel(data) < 8
        error('csv exported by Waveshark contains less than 8 collumns. Bad file type')
    end % if
    % get only last collumn with sampled values
    data = data{8};
    if not(strcmp(data{1}, 'value'))
        error('csv exported by Waveshark does not contain first line with header. Bad file type.')
    end % if
    % find rows with something else than sampled values and remove it
    idx = cellfun(@isempty, data);
    data(idx) = [];
    data(1) = [];       % remove header
    y = cell2mat(cellfun(@(s) sscanf(s, '%f,')', data, 'UniformOutput', false));
end

function proc_SAMU_waveform(udata) %<<<2
    %% Globals %<<<3
    ...
    user_cancel = 0;
            ...
            %     error('proc_SAMU_waveform: The first input argument must be a string with valid file path to the measured data!');
            % end
            % if not(exist(datafile))
            %     error('proc_SAMU_waveform: The data file not found!');
            % end
            %
            % % check algorithm? this can delay everything XXX
            %
            % if not(isnumeric(split))
            %     error('proc_SAMU_waveform: Split must be a number!')
            % end
            % if split < 0
            %     error('proc_SAMU_waveform: Split must be a non-negative number!')
            % end
    ...
    if samples < samples_in_period
        errmsg = sprintf('Error: Data does not contain even single period of the signal, aborting. Samples in the record: %d. Samples in one period of the signal: %d', samples, samples_in_period);
        errordlg(errmsg, GUIname)
        error(errmsg);
    end
...
end % function proc_SAMU_waveform
...
% Calculate result
    ...
    % return 0 or something far away from the estimate, and this cause error. So
    % the estimate is set back to the user's input
    if or(FrequencyEstimate.f.v < tmp.*0.9, FrequencyEstimate.f.v > tmp.*1.1)
        warning('PSFE frequency estimate differs from the user`s estimate by more than 10 \%, setting back to user`s estimate. This happens for N (neutral/shorted/grounded) wire.')
        DI.fest.v = tmp;
        DI.fest.v
    else
    ...
    elseif strcmp(algid, 'PSFE>resamplingSVstream>FFT')             % not good, should be some constant instead of PSFE>>
        ...
        error('proc_SAMU_waveform_gui: mismatch in selected algorithm. Please ask author to fix this.')
    end
